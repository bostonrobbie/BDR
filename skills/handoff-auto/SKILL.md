...
### Before Each Run
1. Read `skills/handoff-auto/learned-patterns.md` if it exists — apply any documented calibration adjustments
2. Count entries in `skills/handoff-auto/run-log.md` to determine current run number
...
3. Overwrite `skills/handoff-auto/learned-patterns.md` with updated findings
4. If a pattern appears in 4+ of 5 runs: write a `## SKILL UPDATE PROPOSAL — handoff-auto` entry to `memory/session/messages.md` for Rob's review
