...
### Before Each Run
1. Read `skills/compliance-gate/learned-patterns.md` if it exists — apply any documented calibration adjustments
2. Count entries in `skills/compliance-gate/run-log.md` to determine current run number
...
3. Overwrite `skills/compliance-gate/learned-patterns.md` with updated findings
4. If a pattern appears in 4+ of 5 runs: write a `## SKILL UPDATE PROPOSAL — compliance-gate` entry to `memory/session/messages.md` for Rob's review
