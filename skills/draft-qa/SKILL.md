...
### Before Each Run
1. Read `skills/draft-qa/learned-patterns.md` if it exists — apply any documented calibration adjustments
2. Count entries in `skills/draft-qa/run-log.md` to determine current run number
...
3. Overwrite `skills/draft-qa/learned-patterns.md` with updated findings
4. If a pattern appears in 4+ of 5 runs: write a `## SKILL UPDATE PROPOSAL — draft-qa` entry to `memory/session/messages.md` for Rob's review
